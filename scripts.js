...
const lowerCaseWords = (mixedArray) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (mixedArray.length >= 0 && Array.isArray(mixedArray)) {
      ...
      const filteredArrayHavingOnlyStr = mixedArray.filter(
        (eachElOfArr) => typeof eachElOfArr === "string"
      );
      ...
      const mappedArrayOfLowerCaseStrings = filteredArrayHavingOnlyStr.map(
        (eachItemOfFilterArr) => eachItemOfFilterArr.toLowerCase()
      );
